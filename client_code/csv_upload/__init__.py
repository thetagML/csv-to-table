...
class csv_upload(csv_uploadTemplate):
  ...
  def file_loader_1_change(self, file, **event_args):
    ...
    if file:
      ...
      getattr(self, RadioButton(group_name="radioGroup_processing").get_group_value())(
        file
      )
  ...
  def process_file_browser(self, csv_file):
    ...
    self.parse_data = alert(
      header_selection(
        header_list=header_list, has_headers=self.check_box_headers_toggle.checked
      ),
      large=True,
    ...
    if not self.check_box_headers_toggle.checked:
      ...
      readlines = (
        gen
        for gen_concat in ((x for x in [raw_header]), readlines)
        for gen in gen_concat
      )
    ...
    with anvil.server.no_loading_indicator as l:
      for line_num, line in enumerate(readlines):
        ...
        try:
          insert_row = {
            col: parse(v)
            for (col, parse), v in zip(self.parse_data, insert_row)
        ...
        try:
          ...
          self.print_to_console(
            f"Error in the following row ({err}):\n{insert_row}\n------------------"
          )
        ...
        if line_num % 1000 == 0 and line_num != 0:
          ...
          new_time = time.time()
          self.print_to_console(
            f"Parsed 1000 lines. ({line_num}) {round(new_time - old_time, 4)} seconds have elapsed. (approx. {round( line_num / ((new_time - old_time) / 60), 4)} rows per min.)"
          )
          self.print_to_console(
            "Working.",
          )
    ...
    self.print_to_console(
      f"Total process time was {floor(( new_time - old_time ) / 60)} minutes and {round((new_time - old_time) % 60,4)} seconds."
    )
    return
  ...
  def process_file_server_module(self, csv_file):
    ...
    with anvil.server.no_loading_indicator as l:
      for line_num, line in enumerate(readlines):
        ...
        if len(row_cashe) > int(self.text_box_max_rows.text):
          ...
          if not self.send_rows_to_server(self.add_table, row_cashe):
            self.text_box_max_rows.text = int(
              int(self.text_box_max_rows.text)
              * (0.5 if not self.check_box_pause_automatic_increase.checked else 0.85)
            )
            time.sleep(1)
            self.send_rows_to_server(
              self.add_table, row_cashe[: int(self.text_box_max_rows.text)]
            )
          ...
          time.sleep(0.35)
          self.print_to_console(
            f" (Server Time {round(server_new_time - server_old_time, 4)} seconds.)",
            newline=False,
          )
      ...
      if row_cashe and not self.abort_operation:
        ...
        server_old_time = time.time()
        if not self.send_rows_to_server(self.add_table, row_cashe):
          self.text_box_max_rows.text = int(int(self.text_box_max_rows.text) * 0.5)
          time.sleep(1)
          self.send_rows_to_server(
            self.add_table, row_cashe[: int(self.text_box_max_rows.text)]
          )
  ...
  def send_rows_to_server(self, table, rows):
    try:
      ...
      self.print_to_console(
        f"Too many rows, reducing {len(rows)} row submission by {(50 if not self.check_box_pause_automatic_increase.checked else 15)}%"
      )
      return False
